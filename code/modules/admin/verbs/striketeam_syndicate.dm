...
//		return
	...
	var/input = null
	while(!input)
		input = copytext_char(sanitize(input(src, "Please specify which mission the syndicate strike team shall undertake.", "Specify Mission", "")),1,MAX_MESSAGE_LEN)
		if(!input)
...
/mob/living/carbon/human/proc/equip_syndicate_commando(syndicate_leader_selected = 0)
	...
	equip_to_slot_or_del(new /obj/item/ammo_magazine/external/sm45(src), slot_in_backpack)
	...
	equip_to_slot_or_del(new /obj/item/weapon/gun/projectile/automatic/pistol/silenced/usp(src), slot_belt)
