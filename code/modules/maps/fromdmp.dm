...
proc/dmp2swapmap(filename)
	...
	while(txt)
		if(text2ascii(txt)==34)
			...
			if(!i)
				world << "Corrupt map file [filename]: No matching ) for coordinates: [copytext_char(txt,1,findtext(txt,"\n"))]"
				return
			var/list/coords=d2sm_ParseCommaList(copytext_char(txt,2,i))
			if(istext(coords) || coords.len!=3)
				world << "Corrupt map file [filename]: [istext(coords)?(coords):"[copytext_char(txt,1,i+1)] is not a valid (x,y,z) coordinate"]"
				return
			j=findtext(txt,"{",i+1)
			if(!j)
				world << "Corrupt map file [filename]: No braces {} following [copytext_char(txt,1,i+1)]"
				return
			k=d2sm_MatchBrace(txt,j,123)
			if(!k)
				world << "Corrupt map file [filename]: No closing brace } following [copytext_char(txt,1,i+1)]"
				return
			var/mtxt=copytext_char(txt,j+1,k)
			if(findText(mtxt,"\"\n")!=1 || !findText(mtxt,"\n\"",length(mtxt)-1))
				world << findText(mtxt,"\"\n")
				world << findText(mtxt,"\n\"",length(mtxt)-1)
				world << "Corrupt map file [filename]: No quotes in braces following [copytext_char(txt,1,i+1)]"
				return
