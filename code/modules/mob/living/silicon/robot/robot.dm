	...
	if(istype(user,/mob/living/carbon/human))
		...
		if(H.species.can_shred(H))
			...
			if (prob(90))
				...
				for(var/mob/O in viewers(src, null))
					O.show_message(text("\red <B>[user] has slashed at [src]!</B>"), 1)
				if(prob(8))
					flick("noise", flash)
				adjustBruteLoss(damage)
				updatehealth()
				...
				for(var/mob/O in viewers(src, null))
					if ((O.client && !( O.blinded )))
						O.show_message(text("\red <B>[user] took a swipe at [src]!</B>"), 1)
			return
...
/mob/living/silicon/robot/verb/pose()
	...
	pose =  copytext_char(sanitize(input(usr, "This is [src]. It is...", "Pose", null)  as text), 1, MAX_MESSAGE_LEN)
