...
//					log_debug("[src] : Exhaling some viruses")
	...
	proc/handle_breath(datum/gas_mixture/breath)
		...
		if( (breath.temperature < species.cold_level_1 || breath.temperature > species.heat_level_1) && !(COLD_RESISTANCE in mutations))
			...
			if(clamp(breath.temperature, -INFINITY, species.cold_level_3))
				apply_damage(COLD_GAS_DAMAGE_LEVEL_3, BURN, "head", used_weapon = "Excessive Cold")
				fire_alert = max(fire_alert, 1)
			if(clamp(breath.temperature, species.cold_level_3, species.cold_level_2))
				apply_damage(COLD_GAS_DAMAGE_LEVEL_2, BURN, "head", used_weapon = "Excessive Cold")
				fire_alert = max(fire_alert, 1)
			if(clamp(breath.temperature, species.cold_level_2, species.cold_level_1))
				apply_damage(COLD_GAS_DAMAGE_LEVEL_1, BURN, "head", used_weapon = "Excessive Cold")
				fire_alert = max(fire_alert, 1)
			if(clamp(breath.temperature, species.heat_level_1, species.heat_level_2))
				apply_damage(HEAT_GAS_DAMAGE_LEVEL_1, BURN, "head", used_weapon = "Excessive Heat")
				fire_alert = max(fire_alert, 2)
			if(clamp(breath.temperature, species.heat_level_2, species.heat_level_3))
				apply_damage(HEAT_GAS_DAMAGE_LEVEL_2, BURN, "head", used_weapon = "Excessive Heat")
				fire_alert = max(fire_alert, 2)
			if(clamp(breath.temperature, species.heat_level_3, INFINITY))
...
//			world << "Environment: [loc_temp], [src]: [bodytemperature], Adjusting: [temp_adj]"
		...
		if(bodytemperature > species.heat_level_1)
			...
			if(clamp(bodytemperature, species.heat_level_1, species.heat_level_2))
				...
				fire_alert = max(fire_alert, 2)
			if(clamp(bodytemperature, species.heat_level_2, species.heat_level_3))
				if(on_fire)
					take_overall_damage(burn=HEAT_DAMAGE_LEVEL_3, used_weapon = "Skin Burns")
					fire_alert = max(fire_alert, 2)
				else
					take_overall_damage(burn=HEAT_DAMAGE_LEVEL_2, used_weapon = "High Body Temperature")
					fire_alert = max(fire_alert, 2)
			if(clamp(bodytemperature, species.heat_level_3, INFINITY))
			...
			if(!istype(loc, /obj/machinery/atmospherics/unary/cryo_cell))
				if(clamp(bodytemperature, species.cold_level_2, species.cold_level_1))
					...
					fire_alert = max(fire_alert, 1)
				if(clamp(bodytemperature, species.cold_level_3, species.cold_level_2))
					take_overall_damage(burn=COLD_DAMAGE_LEVEL_2, used_weapon = "Low Body Temperature")
					fire_alert = max(fire_alert, 1)
				if(clamp(bodytemperature, -INFINITY, species.cold_level_3))
	...
	proc/handle_chemicals_in_body()
			...
			if(isturf(loc)) //else, there's considered to be no light
				...
				if(A)
					if(A.ul_Lighting)
						...
						light_amount = min(10,light_amount) - 5 //hardcapped so it's not abused by having a ton of flashlights
					else
						light_amount =  5
		...
		if(dna && dna.mutantrace == "shadow")
			...
			if(isturf(loc))
				...
				var/area/A = T.loc
				if(A)
					if(A.ul_Lighting)	light_amount = (T.ul_GetRed() + T.ul_GetGreen() + T.ul_GetBlue())/3
