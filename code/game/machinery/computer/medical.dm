		...
		if (src.authenticated)
			...
			if (href_list["field"])
				...
				switch(href_list["field"])
					...
					if("notes")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize(input("Please summarize notes:", "Med. records", src.active2.fields["notes"], null)  as message)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
					...
					if("b_dna")
						if (istype(src.active1, /datum/data/record))
							var/t1 = reject_bad_text(sanitize(input("Please input DNA hash:", "Med. records", src.active1.fields["dna"], null)  as text))
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active1 != a1))
								return
					...
					if("vir_name")
						...
						if (v)
							var/t1 = reject_bad_name(input("Please input pathogen name:", "VirusDB", v.fields["name"], null)  as text,1)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active1 != a1))
								return
							v.fields["name"] = t1
					if("vir_desc")
			...
			if (href_list["add_c"])
				if (!( istype(src.active2, /datum/data/record) ))
					return
				var/a2 = src.active2
				var/t1 = copytext_char(sanitize_PDA(input("Add Comment:", "Med. records", null, null)  as message),1,MAX_MESSAGE_LEN)
				if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
					return
				var/counter = 1
				while(src.active2.fields[text("com_[]", counter)])
			...
			if (href_list["search"])
				var/t1 = reject_bad_text(input("Search String: (Name, DNA, or ID)", "Med. records", null, null)  as text)
