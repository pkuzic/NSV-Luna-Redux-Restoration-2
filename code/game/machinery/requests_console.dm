...
/obj/machinery/requests_console/Topic(href, href_list)
	...
	if(reject_bad_text(href_list["write"]))
		...
		var/new_message = copytext_char(reject_bad_text(input(usr, "Write your message:", "Awaiting Input", "")),1,MAX_MESSAGE_LEN)
		if(new_message)
			message = new_message
			screen = 9
	...
	if(href_list["writeAnnouncement"])
		var/new_message = sanitize(copytext_char(input(usr, "Write your message:", "Awaiting Input", ""),1,MAX_MESSAGE_LEN))
		if(new_message)
			message = new_message
			switch(href_list["priority"])
				if("2")	priority = 2
