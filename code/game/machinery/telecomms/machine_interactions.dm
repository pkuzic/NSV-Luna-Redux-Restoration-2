...
/obj/machinery/telecomms/attack_hand(var/mob/user as mob)
	...
	if(on && toggled)
		...
		dat += "<br>d_filtering Frequencies: "
		...
		dat += "<br>  <a href='?src=\ref[src];input=freq'>\[Add d_filter\]</a>"
		dat += "<hr>"
...
/obj/machinery/telecomms/Topic(href, href_list)
	...
	if(href_list["input"])
		switch(href_list["input"])
			...
			*/
			...
			if("freq")
				var/newfreq = input(usr, "Specify a new frequency to d_filter (GHz). Decimals assigned automatically.", src, network) as null|num
				if(newfreq && canAccess(usr))
					if(findtext(num2text(newfreq), "."))
						newfreq *= 10 // shift the decimal one place
					if(!(newfreq in freq_listening) && newfreq < 10000)
						freq_listening.Add(newfreq)
						temp = "<font color = #666633>-% New frequency d_filter assigned: \"[newfreq] GHz\" %-</font color>"
	...
	if(href_list["delete"])
		...
		temp = "<font color = #666633>-% Removed frequency d_filter [x] %-</font color>"
