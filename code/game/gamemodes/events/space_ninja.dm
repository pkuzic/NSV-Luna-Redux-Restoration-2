		...
		*/
			...
			if(!ninja_mind.objectives.len||!mission_set)//If they somehow did not get an objective at this point, time to destroy the station.
				...
				if(nuke_code)//If there is a nuke device in world and we got the code.
					...
					ninja_objective.owner = ninja_mind
					ninja_objective.explanation_text = "Destroy the [vessel_type] with a nuclear device. The code is [nuke_code]." //Let them know what the code is.
...
/proc/generate_ninja_directive(side)
	...
	switch(rand(1,19))
		...
		if(2)
			directive += "[vessel_name] is financed by an enemy of the Spider Clan. Cause as much structural damage as desired."
		if(3)
		...
		if(6)
			directive += "We are engaged in a legal dispute over [vessel_name]. If a laywer is present on board, force their cooperation in the matter."
		if(7)
...
/client/proc/send_space_ninja()
	...
	var/mission
	while(!mission)
		mission = sanitize_uni(input(src, "Please specify which mission the space ninja shall undertake.", "Specify Mission", ""))
		if(!mission)
